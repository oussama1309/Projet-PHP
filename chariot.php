...
<!-- Basic -->
    ...
    <body>
                ...
                <div class="container">
                    ...
                    <div class="collapse navbar-collapse" id="navbar-menu">
                        <ul class="nav navbar-nav ml-auto" data-in="fadeInDown" data-out="fadeOutUp">
                            <li class="nav-item"><a class="nav-link" href="acceuil-client.php">Accueil</a></li>
        ...
        <div class="chariot container">
                            ...
                            while($chariot=$ch->fetch()){
                                if($chariot['cid']==$cid){
                                    ...
                                    while($produit=$pr->fetch()){
                                        if($produit['pid']==$chariot['pid']){
                                            $pid=$produit['pid'];
                                            $prixtot+=($produit['prix']*$chariot['qtt']);
                                            echo '
                                                <tr>
                                                    <form action="php/misajour-chariot.php?cid='.$cid.'&pid='.$produit['pid'].'" method="POST">
                                                        <td class="thumbnail-img">
                                                            <a href="produit.php?cid='.$cid.'&pid='.$pid.'">
                                                                <img src="data:image/jpeg;base64,'.base64_encode($produit['file']).'" class="img-ch img-fluid" alt="" />
                                                            </a>
                                                        </td>
                                                        <td class="name-pr">
                                                            <a href="produit.php?cid='.$cid.'&pid='.$pid.'">
                                                        ...
                                                        <td class="quantity-box">
                                                            <input type="number" name="qtt" size="4" value="'.$chariot['qtt'].'" min="0" step="1" class="c-input-text qty text">
                                                        </td>
                                                        <td class="total-pr">
                                                            <p>'.$produit['prix']*$chariot['qtt'].' DT</p>
                                                        </td>
                                                        <td class="remove-pr">
                                                            <a href="#"><i class="fas fa-times"></i></a>
                                                        </td>
                                                        <td>
                                                            <input type="submit" name="mis" Value="Mise à jour">
                                                        </td>
                                                    </form>
                                            ';
                                            /*if(isset($_POST['mis'])){
                                                $qtt=$_POST['qtt'];
                                                $crt=$cnx->query('SELECT * FROM chariot');
                                                while($cart=$crt->fetch()){
                                                    if(($chariot['cid']==$cid)&&($chariot['pid']==$pid)){
                                                        $cnx->query('UPDATE `chariot` SET `qtt`='.$qtt.' WHERE `pid`='.$pid.' AND `cid`='.$cid.'');
                                                    }
                                                }
                                            }*/
                                            echo'
            ...
            <!---->
            <form action="commande-ch.php?cid=<?php echo $cid;?>" method="post">
                <div class="row my-5">
                    <div class="col-lg-2 col-sm-12">
                        <div class="order-box">
                            <div class="d-flex gr-total">
                                <input type="checkbox" name="checkliv" id="liv" value="0">
                                <label for="checkliv">Livrer ces commandes</label>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6 col-sm-12">
                        <div class="order-box">
                            <div class="d-flex gr-total">
                                <h5>Grand Total</h5>
                                <div class="ml-auto h5"><?php echo ' '.$prixtot.' DT'; ?></div>
                            </div>
                        </div>
                    </div>
